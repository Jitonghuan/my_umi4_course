...
function Tracking() {
    ...
    // 降噪数据前端过滤处理
    var filteredSpans = react_1.useMemo(function () {
        var copyData = JSON.parse(JSON.stringify(rightData));
        var filterOptioins = noiseList
            .filter(function (item) { return item.noiseReductionMeasure === "ignore"; })
            .map(function (e) { return e.noiseReductionComponent; });
        var filter = function (arr, options) {
            if (!(filterOptioins === null || filterOptioins === void 0 ? void 0 : filterOptioins.length)) {
                return arr;
            }
            return arr.filter(function (span) {
                var _a;
                if ((_a = span.children) === null || _a === void 0 ? void 0 : _a.length) {
                    span.children = filter(span.children, options);
                }
                return !options.includes(span.component);
            });
        };
        var mergeOptioins = noiseList
            .filter(function (item) { return item.noiseReductionMeasure === 'merge'; })
            .map(function (e) { return e.noiseReductionComponent; });
        var merge = function (arr, options) { return arr.reduce(function (res, span) {
            var _a;
            if (options.includes(span.component) && res.length !== 0) {
                var prew = res[res.length - 1];
                if (prew.component === span.component) {
                    if (!prew.isMerged) {
                        prew.isMerged = true;
                        prew.endpointName = "Merged " + span.component;
        ...
        return merge(filter(copyData, filterOptioins), mergeOptioins);
    }, [rightData, noiseList]);
    //获取环境列表
    ...
    // 降噪下拉框发生改变时
    var noiseChange = function (value, options) {
        var selectOptions = options.filter(function (item) { return value.includes(item.id); });
    ...
    var queryTraceList = function (params) {
        ...
        var noiseIdList = localStorage.getItem('trace_noise_list') || '';
        var values = form.getFieldsValue();
        var start = moment_1["default"](selectTime.start).format('YYYY-MM-DD HH:mm:ss');
        var end = moment_1["default"](selectTime.end).format('YYYY-MM-DD HH:mm:ss');
        service_1.getTrace(__assign(__assign(__assign({}, params), values), { end: end, start: start, envCode: selectEnv, noiseReductionIDs: noiseIdList.split(',') }))
            .then(function (res) {
            var _a, _b, _c, _d, _e;
            if (res) {
    ...
    var queryTreeData = function () {
        if (!(currentItem === null || currentItem === void 0 ? void 0 : currentItem.traceIds) || !(currentItem === null || currentItem === void 0 ? void 0 : currentItem.traceIds[0]))
            return;
        setRightLoading(true);
        service_1.getTraceInfo({ traceID: currentItem === null || currentItem === void 0 ? void 0 : currentItem.traceIds[0], envCode: selectEnv })
            .then(function (res) {
            var _a, _b;
    ...
    return (React.createElement(page_container_1["default"], { className: "tracking-container" },
        React.createElement(vc_page_content_1.ContentCard, { className: "trace-detail-page", style: { height: '100%' } },
            ...
            React.createElement("div", { className: "search-form", style: { marginBottom: '20px' } },
                React.createElement(antd_1.Form, { layout: "inline", form: form, onFinish: function () {
                    ...
                    expand && (React.createElement(antd_1.Form.Item, { label: "\u5E94\u7528", name: "appID" },
                        React.createElement(antd_1.Select, { value: appID, options: applicationList, onChange: function (value) {
                                setAppID(value);
                            }, allowClear: true, showSearch: true, optionFilterProp: "label", filterOption: function (input, option) {
                                var _a;
                    ...
                    expand && (React.createElement(antd_1.Form.Item, { label: "\u5B9E\u4F8B", name: "instanceCode" },
                        React.createElement(antd_1.Select, { options: instanceList, showSearch: true, allowClear: true, style: { width: 150 }, optionFilterProp: "label", filterOption: function (input, option) {
                                var _a;
                                return ((_a = option === null || option === void 0 ? void 0 : option.label) === null || _a === void 0 ? void 0 : _a.toLowerCase().indexOf(input.toLowerCase())) >= 0;
                            } }))),
            ...
            React.createElement("div", { className: "detail-main" }, first ? (React.createElement("div", { className: "empty-holder" }, "\u8BF7\u70B9\u51FB\u67E5\u8BE2\u8FDB\u884C\u641C\u7D22")) : (React.createElement(resiable_pro_1["default"], { leftComp: React.createElement(left_list_1["default"], { listData: listData || [], total: total, loading: loading, changeItem: leftItemChange, pageChange: queryTraceList }), rightComp: (filteredSpans === null || filteredSpans === void 0 ? void 0 : filteredSpans.length) !== 0 || rightLoading ? (React.createElement(right_trace_1["default"], { item: currentItem || {}, data: filteredSpans, envCode: selectEnv, selectTime: selectTime, noiseChange: noiseChange, loading: rightLoading })) : (React.createElement(antd_1.Empty, { image: antd_1.Empty.PRESENTED_IMAGE_SIMPLE, style: { width: '100%', overflow: 'hidden' } })), isShowExpandIcon: true, defaultClose: true, leftWidth: 240 }))))));
}
