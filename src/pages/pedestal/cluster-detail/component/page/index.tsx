...
export default function page(props: any) {
    const { continueList, clickLeft, clickRright, total = 10, pageIndex = 1, totalPage = 1, pageChange, defaultValue } = props;
    const leftDisable = useMemo(() => pageIndex <= 1, [pageIndex]);
    const rightDisable = useMemo(() => totalPage == pageIndex, [pageIndex, totalPage]);
    const pageOptions = [{ label: '20/页', value: 20 }, { label: '50/页', value: 50 }, { label: '全部', value: '' }]
    return (
        <div className='flex-row'>
            ...
            <div style={{ marginLeft: '5px' }}>
                <Select options={pageOptions} defaultValue={defaultValue} onChange={pageChange} style={{ width: 80 }}>
                </Select>
            </div>
        </div>
    )
