...
import { deployMaster, newDeployMaster } from '@/pages/application/service';
interface Iprops {
...
export default function EntryProject(props: Iprops) {
  const { deployMasterVisible, onClose, onSave, masterBranchOptions, selectMaster, changeMaster, envDataList, curPipelineCode, deployModel, feType, appData, newPublish = false } = props;
  ...
  const confirmPublishToMaster = async () => {
    ...
    try {
      const deploy = newPublish ? newDeployMaster : deployMaster;
