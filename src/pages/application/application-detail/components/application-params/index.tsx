...
export default function ApplicationParams(
  ...
  const [selectEnvData, setSelectEnvData] =
    useState<string>(''); //下拉选择应用环境
  const [selectTmpl, setSelectTmpl] =
    useState<string>(''); //下拉选择应用模版
  const [applicationlist, setApplicationlist] =
    useState<any>([]); //获取到的结果
  const [inintDatas, setInintDatas] =
    useState<any>([]); //初始化的数据
  const [id, setId] = useState<string>();
  const [isDeployment, setIsDeployment] =
    useState<string>();
  const [ensureDisable, setEnsureDisable] =
  ...
  // 进入页面显示结果
  const { appCode, appCategoryCode } =
    appData || {};
  const { templateType, envCode } =
    ...
    return getRequest(APIS.paramsList, {
      ...
      .then((result) => {
        if (result.data.length > 0) {
          ...
          message.info(
            `${envCode}环境的${templateType}类型模版为空`,
          );
        }
      })
      .finally(() => {
  ...
  const inintData = () => {
    ...
    for (const key in inintDatas.tmplConfigurableItem) {
      if (key === 'jvm') {
        jvm =
          inintDatas.tmplConfigurableItem[key];
      } else {
        arr1.push({
          key: key,
          value:
  ...
  //点击查询回调
  const queryTmpl = async (
    envCodeCurrent: string,
    templateTypeCurrent: string,
  ) => {
    setInfoloading(true);
    ...
    await getRequest(APIS.paramsList, {
      ...
      .then((result) => {
        ...
        if (result.data.length !== 0) {
          ...
          message.error(
            `${envCodeCurrent}环境的${templateTypeCurrent}类型模版不存在,请先推送模板！`,
          );
        }
      })
      .finally(() => {
  ...
  const disabledDateTime = (date: any) => {
    ...
    if (
      ...
      return {
        disabledHours: () =>
          range(0, 24).splice(0, hours),
        disabledMinutes: () =>
          range(0, 60).splice(0, minutes + 1),
        disabledSeconds: () =>
  ...
  const setApplication = async () => {
    const params =
      await restarForm.validateFields();
    const values =
      applicationForm.getFieldsValue();
    const tmplConfigurableItem =
      values.tmplConfigurableItem?.reduce(
        (prev: any, el: any) => {
