    ...
    <PageContainer>
      <FilterCard className="layout-compact">
        <Tabs activeKey={activeKey} onChange={(next) => props.history.push(`/matrix/cluster/cluster-zs/${next}`)}>
          <Tabs.TabPane tab="集群看板111" key="dashboards" />
          <Tabs.TabPane tab="流量调度111" key="scheduling-mode" />
          {/* <Tabs.TabPane tab="集群调度" key="scheduling" /> */}
          <Tabs.TabPane tab="集群同步111" key="cluster-sync" />
          <Tabs.TabPane tab="应用同步111" key="application-sync" />
          <Tabs.TabPane tab="操作记录11" key="operation-log" />
        </Tabs>
      </FilterCard>
